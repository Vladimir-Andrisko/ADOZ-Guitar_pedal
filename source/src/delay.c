...
static uint16_t current_delay = 0;
...
void delay_init(uint16_t N){
    ...
    current_delay = N;
    dbuff_read_index = 0;
    dbuff_write_index = (dbuff_read_index + current_delay) & MAX_DELAY;
}
...
void set_echo_delay(uint16_t N){
    ...
    current_delay = N;
    dbuff_read_index = (dbuff_write_index + DELAY_BUFFER_SIZE - current_delay) & MAX_DELAY;
}
...
int32_t echo_sample(int32_t input)
{
    ...
    int32_t acc = input;
    acc += (int32_t)lrintf(echo_gain * (float)delayed);
...
int32_t reverb_sample(int32_t input){
    ...
    int32_t acc = input;
    acc += (int32_t)lrintf(reverb_gain * (float)delayed);
    if (acc > INT32_MAX) acc = INT32_MAX;
    if (acc < INT32_MIN) acc = INT32_MIN;
